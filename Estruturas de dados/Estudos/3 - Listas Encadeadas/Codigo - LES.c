...
/* obtem o dado que está na posição indicada

int Maiorque_Dados_LS (Tno_ls *c_inicio, int info, int *quant);
/* obtem a quantidade de dados maior que info */

int Permuta_Dados_LS (Tno_ls *c_inicio, int pos1, int pos2);
/* permuta os dados que estão nas posições pos1 e pos2 */
	...
	do {
		...
		switch(q) {
			case 1: printf("Dado para insercao na lista: ");
                    ...
                    system("pause");
					break;
            case 12: printf("Dado para insercao na lista: ");
                    scanf("%d",&info);
                    printf("Posicao de insercao: ");
                    scanf("%d",&pos);
                    erro=Inserir_meio_LS (&ini, info, pos);
                    if (erro == 0) printf("Insercao realizada com sucesso\n");
                    else printf("Insercao nao realizada\n");
                    system("pause");
					break;
...
{
  // A partir de uma valor, retorna a posição deste na lista
  *pos = 0;
  while (c_inicio != NULL)
  {
     (*pos)++;
